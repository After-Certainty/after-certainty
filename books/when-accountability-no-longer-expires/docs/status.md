# When Accountability No Longer Expires — Drafting Status

## Current phase

**Phase 5 — promoted to `books/when-accountability-no-longer-expires/` (essay edition)**

## Active branch

`cursor/upcoming-pass-4-editorial-quick-wins-9a98`

## Manuscript hub

[`index.md`](../index.md)

## Unit progress

| Unit | Phase | Notes |
|------|-------|-------|
| Title Page | draft | |
| Copyright | draft | |
| Author's Note | draft | |
| Preface | draft | Pandoc footnotes |
| Introduction | draft | |
| Ch 1 — Authority Without an Expiration Date | draft | Pandoc footnotes |
| Ch 2 — Sacralization and Inheritance | draft | Pandoc footnotes |
| Ch 3 — Harm Without Villains | draft | Pandoc footnotes |
| Ch 4 — The Distribution of Suffering | draft | Pandoc footnotes |
| Ch 5 — Moral Drift Without Collapse | draft | Pandoc footnotes |
| Bridge — Learning Without Resolution | draft | Pass 4 continuity + echo trim |
| Ch 6 — Adaptation Is Not Renewal | draft | Pandoc footnotes |
| Ch 7 — Correction Without Consequence | draft | Pandoc footnotes |
| Ch 8 — Why This Feels Like Wisdom From the Inside | draft | Pandoc footnotes (Pass 4) |
| Ch 9 — Warning Is Not Learning | draft | Pandoc footnotes |
| Ch 10 — The Stable Equilibrium | draft | Pandoc footnotes (Pass 4) |
| Interlude | draft | Pass 4 handoff to Part III bridge |
| Bridge — Beyond Any One Institution | draft | Pass 4; split-marker removed |
| Ch 11 — The Pattern Across Domains | draft | |
| Ch 12 — Why These Patterns Are Not Inevitable | draft | Pandoc footnotes |
| Conclusion | draft | |
| Appendix A | draft | Accountability vs blame |
| Bibliography | draft | Chicago-style; linked from index |

## Author gate

Checklist: [`author-read-through-gate.md`](author-read-through-gate.md) — complete sign-off table when read-through is done.

## Next actions

1. Optional post-promote: bridge → interlude → Part IV vocabulary pass after export smoke test.
2. Source of truth for edits: [`books/when-accountability-no-longer-expires/`](../../books/when-accountability-no-longer-expires/docs/status.md).

## Open decisions / known issues

None recorded.

## Rough scale

- ~12,400 words (May 2026; post Pass 4 bridges)
