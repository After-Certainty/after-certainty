# The Relay — documentation index

**Start here** when drafting or revising.

## Authority (read in order)

| Doc | Use |
|-----|-----|
| **[project-spec.md](./project-spec.md)** | Premise, dual mystery, manuscript rules, ending, anti-patterns |
| **[character-bible.md](./character-bible.md)** | Cast, sees/misses, independent goals, Arin |
| **[civilization-bible.md](./civilization-bible.md)** | Everyday life — worlds, travel, jobs, myths, wrong beliefs |
| **[iconic-locations.md](./iconic-locations.md)** | Place bible — Ribbon culture, five anchors |
| **[symbol-migration-plan.md](./symbol-migration-plan.md)** | Relay symbolic architecture (replaces maintenance-mark motifs) |
| **[motifs.md](./motifs.md)** | Quick reference → migration plan |
| **[voice-spec.md](./voice-spec.md)** | Prose, dialogue, implication craft |
| **[synopsis.md](./synopsis.md)** | Living plot spine |
| **[act-chapter-index.md](./act-chapter-index.md)** | Full novel chapter plan (~27) |
| **[act-chapter-stubs.md](./act-chapter-stubs.md)** | Scene-level stubs for all chapters |
| **[mystery-beat-sheet.md](./mystery-beat-sheet.md)** | Dual mystery + wrong theory at 90–100k |
| **[full-manuscript-developmental-review.md](./full-manuscript-developmental-review.md)** | Expansion priorities (wonder, gravity, voice) |
| **[act-2-agent-pass-report.md](./act-2-agent-pass-report.md)** | Act II expansion + agent results |
| **[agents/21-contemporary-dialogue-agent.md](./agents/21-contemporary-dialogue-agent.md)** | TV/audiobook dialogue; anti-thesis speech |
| **[agents/22-author-intrusion-agent.md](./agents/22-author-intrusion-agent.md)** | Narrator-as-editor flags; report only |
| **[agents/23-redundant-explanation-agent.md](./agents/23-redundant-explanation-agent.md)** | Retell after show; delete test; report only |
| **[agents/README.md](./agents/README.md)** | Cursor revision agents (incl. Brittany, Language drift) |

## Drafting & revision

| Doc | Use |
|-----|-----|
| **[exemplar-chapter-01-drafting.md](./exemplar-chapter-01-drafting.md)** | Habits from Lena Ch. 1 |
| **[exploratory-chapter-feedback.md](./exploratory-chapter-feedback.md)** | Reader assessment from probe phase (archived) |
| **[chapter-edit-pass.md](./chapter-edit-pass.md)** | Human checklist |
| **[status.md](./status.md)** | Progress tracker |
