...
This is a **design memory**. The seven guides plus the synopsis, chapter outline, and working notebooks are now the canonical docs for the novel, living here in `docs/`. The book's earlier canon (the project-spec, character/civilization bibles, voice-spec, and the old mystery beat sheet) has been superseded by this handbook and removed.
...
The superseded canon (the old project-spec and synopsis) was built on *maintaining* a civilization, a *discovery* mystery, and a cast of **complementary lenses** — additive "puzzle-piece" thinking that the new direction rejects.
...
- **Centerpiece (inside Integration):** positions → underlying interests → the invented third solution none proposed (Follett). A distributed-knowledge map — how fragments combine into knowledge — replaces the old mystery beat sheet, expressed across the five movements.
...
- **Purpose:** The prose/voice/tone bible governing drafting; supersedes the old voice-spec.
...
**Calibration is done — the two chapters are now the register touchstone.** [`Chapter 1 — Suspended Pending Review`](../manuscript/act-1-the-absence/chapter-01-suspended-pending-review.md) and [`Chapter 9 — The Best Man in the Room`](../manuscript/act-2-the-cascade/chapter-09-the-best-man-in-the-room.md) define how the book sounds; when a later chapter feels off-register, read the nearer of the two before reopening any guide. What they *proved* has been folded back into the system in three small places (and nowhere else — the cure for weight is still lighter use, not more documents):
