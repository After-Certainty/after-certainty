...
A school that cannot hear poor formation may still hear test performance. A child improves on the state assessment while becoming less curious, less patient, less willing to remain with a problem that does not resolve quickly. The score travels. The teacher's private observation—that the child is learning to succeed without learning to stay—does not. When the observation finally surfaces, it is often translated into intervention language the system already owns: tutoring, remediation, behavioral supports. Each may help. None automatically asks whether the measurable curriculum has been crowding out the other curriculum formation requires.
