...
npm run site:dev          # Next.js site workspace
...
Site CI runs from [`.github/workflows/site-ci.yml`](.github/workflows/site-ci.yml) (path-filtered): PR preview deploys start immediately in parallel with lint/tests; E2E hits the deployed preview URL; `site-ci` aggregates the merge gate. Local DX: `npm run site:dev:local` / `npm run site:dev:watch` / `npm run site:build:local` (Turbo: manifest → install → Next). Production still waits for quality + E2E before `vercel deploy --prebuilt --prod` ([`docs/site-deploy.md`](docs/site-deploy.md); [`apps/site/vercel.json`](apps/site/vercel.json)). The former standalone site repository is [archived](https://github.com/ksteffe/after-certainty-site).
