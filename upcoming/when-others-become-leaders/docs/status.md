# When Others Become Leaders — Drafting Status

## Current phase

**Phase 2 — Unit editorial passes (in progress)**

Working in manuscript order: rules, echo, editorial, citation, and linkage check per unit.

## Active branch

`cursor/when-others-become-leaders-planning-ba72`

## Manuscript hub

[`index.md`](../index.md) is the source of truth for manuscript reading order.

## Key docs

- [`README.md`](README.md)
- [`book-overview.md`](book-overview.md)
- [`outline.md`](outline.md)
- [`voice-guide.md`](voice-guide.md)
- [`drafting-process.md`](drafting-process.md)
- [`bibliography-guide.md`](bibliography-guide.md)
- [`research/README.md`](research/README.md)
- Portfolio rollup: [`upcoming/docs/portfolio-status.md`](../../docs/portfolio-status.md)

## Edition policy

| Decision | Status | Notes |
|----------|--------|-------|
| Target length | **Locked** | ~50,000 words; no expansion band |
| Chapter count | **Locked** | Nine chapters + intro + epilogue |
| Part structure | **Locked** | Three-part arc as drafted |
| WOLTY / circulation | **Locked** | [`reading-with-the-series.md`](../front-matter/reading-with-the-series.md) |

## Unit progress

| Unit | Phase | Words | Notes |
|------|-------|------:|-------|
| What This Book Is | revised | ~130 | Phase 2 pass; series link clarified |
| Reading With the Series | revised | ~450 | Phase 2 pass; WOLTY v1/v2 repo links |
| About the Series | revised | — | Phase 2 pass; pointer to reading-with-the-series |
| Introduction — The Wrong Question | revised | ~2,140 | Phase 2 pass; staccato reflow at pivots; circulation retained |
| Part I bridge | revised | ~75 | Phase 2 pass; expanded hinge from introduction |
| Ch. 1 — The Table | in_progress | ~6,000 | Phase 2 opening pass (through Zacchaeus section pending) |
| Ch. 2 — Salt | draft | ~5,370 | Awaiting Phase 2 |
| Ch. 3 — Victory | draft | ~4,830 | Awaiting Phase 2 |
| Part II bridge | stub | ~30 | Awaiting Phase 2 |
| Ch. 4 — The March | draft | ~5,910 | Awaiting Phase 2 |
| Ch. 5 — The Tree | draft | ~4,060 | Awaiting Phase 2 |
| Ch. 6 — The Sign | draft | ~5,300 | Awaiting Phase 2 |
| Part III bridge | stub | ~30 | Awaiting Phase 2 |
| Ch. 7 — The House | draft | ~3,900 | Awaiting Phase 2 |
| Ch. 8 — The Neighborhood | draft | ~4,790 | Awaiting Phase 2 |
| Ch. 9 — The Bracelet | draft | ~2,540 | Awaiting Phase 2 |
| Epilogue — The Empty Chair | draft | ~4,890 | Awaiting Phase 2 |
| Bibliography | complete | — | Phase 1 |
| Research packets | complete | — | Phase 1 |
| Cover + OG assets | complete | — | — |

## Next actions

1. Continue Phase 2 on Ch. 1 — The Table (remainder of chapter).
2. Ch. 2 — Salt, Ch. 3 — Victory, then Part I coherence gate (Phase 3).
3. Proceed through Parts II and III in manuscript order.

## Open decisions / known issues

- How explicitly to name the recurring architecture in the epilogue.
- How to frame “enduring” for a living figure (Taylor Swift) without overclaim.

## Rough scale

- Manuscript words (current draft): ~50,300
- Last assessed: July 2026 — Phase 2 started (front matter + intro + Part I bridge + Ch. 1 opening)

## Promotion readiness

**R1** — internal/share draft. Path to **R2**: complete Phases 2–4 + author read-through.
