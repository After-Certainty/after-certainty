# Acts II, III & V — revision progress

**Calibration:** **[act-1-calibration-standard.md](./act-1-calibration-standard.md)**  
**Voice / passes:** **[voice-spec.md](./voice-spec.md)**, **[chapter-edit-pass.md](./chapter-edit-pass.md)**  
**Doc index:** **[README.md](./README.md)**

## Goal

Revise **Acts II (7–13)**, **III (14–20)**, and **V (27–30)** to the **Act I + Act IV manual bar**—consequences over mechanics, boundary-violation bond, tighter pacing, cut thesis closers.

**Do not** re-expand toward the old pipeline average (~2,250 words/ch).

## Pipeline baseline (historical — not the target)

Branch `velorum/acts-2-3-expansion-polish` ran Agent 01 expansion + revision 09/10 before manual Act I/IV calibration superseded that bar.

| Act | Pipeline avg/ch | Pipeline total |
|-----|----------------:|---------------:|
| II (7–13) | ~2,258 | 15,804 |
| III (14–20) | ~2,286 | 16,001 |

Per-chapter pipeline counts are in the **Chapter log** baseline column below.

## Revision target

| Act | Target avg/ch | Reference |
|-----|-------------:|-----------|
| II | ~1,600–2,000 | Act I + ch. 6 consequences model |
| III | ~1,600–2,000 | ch. 18 show-don't-thesis; two-level dialogue |
| V | ~1,200–1,800 | Act IV compression + Act I thematic heat |

Compare to **Act I ~1,786 avg** and **Act IV ~1,172 avg** (**act-1-calibration-standard**).

## Revision checklist (per chapter)

- [ ] Single POV per **act-chapter-index**  
- [ ] Bond = **intrusion / cost**, not sync showcase  
- [ ] No redundant **verification loops** for same bond fact  
- [ ] Dialogue **two-level** (surface vs cost/wanting)  
- [ ] **Thesis closers** cut if scene already lands  
- [ ] **Direct camera** + **plain dialogue** (agent **04**, **chapter-edit C2, F**)  
- [ ] **Scene compression** last (revision **05**) if still over target band  
- [ ] Word count in target band or beat-justified  

## Chapter log

| Ch | File | Baseline (pipeline) | Revised | Status |
|----|------|--------------------:|--------:|--------|
| 7 | chapter-07-shared-load.md | 2,163 | 1,741 | **done** — bond cost in fight; oath-bound aftermath; post-fight intrusion |
| 8 | chapter-08-the-road-to-kestrel-gate.md | 2,412 | 1,832 | **done** — montage compression; dependency/need; farmer seed beat |
| 9 | chapter-09-the-magistrate.md | 2,315 | 2,463 | **done** — Halverin/Merrow; operational bond failures; Riven wins a beat |
| 10 | chapter-10-lyra.md | 2,299 | 1,568 | **done** — Lyra earns diagnosis; harm-with-good-posters; compressed dormitory |
| 11 | chapter-11-no-privacy.md | 2,254 | 1,558 | **done** — ownership theme; canal *mine not to tell*; alley trim |
| 12 | chapter-12-the-attack-on-kestrel-gate.md | 2,261 | 2,311 | **done** — bond fracture kills; internal vs operational cost; Halverin *You noticed* |
| 13 | chapter-13-aftermath.md | 2,247 | 2,278 | **done** — narrative capture; judgment beat; city anger; demo locked |

**Act II revised total (7–13):** ~13,751 words · **~1,964 avg/ch** (pipeline baseline ~15,804 / ~2,258 avg) · **Act II complete**

| 14 | chapter-14-the-explanation.md | 2,589 | — | pending |
| 15 | chapter-15-trying-to-fix-it.md | 2,423 | — | pending |
| 16 | chapter-16-lyras-warning.md | 2,343 | — | pending |
| 17 | chapter-17-the-demonstration.md | 2,191 | — | pending |
| 18 | chapter-18-collapse-in-public.md | 2,201 | — | pending |
| 19 | chapter-19-the-silence-after.md | 2,375 | — | pending |
| 20 | chapter-20-lyra-leaves.md | 2,181 | — | pending |
| 27–30 | act-5-the-legend/* | — | — | pending |

Update as chapters complete manual revision (same workflow as Act I / IV).
