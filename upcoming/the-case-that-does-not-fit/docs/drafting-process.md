# The Case That Does Not Fit — Drafting Process

## Purpose

Workflow for building and revising **The Case That Does Not Fit** in a structured, review-driven way.

## Key references

- `docs/book-rules.md` — house rules (wins on conflict)
- `docs/status.md` — unit progress and next actions
- `docs/outline.md` — canonical chapter architecture from parking-place outline
...
- `upcoming/the-case-that-does-not-fit-structure`
- `upcoming/the-case-that-does-not-fit-draft`
- `upcoming/the-case-that-does-not-fit-editorial`
...
- Keep planning briefs aligned with `docs/outline.md`.

### Phase 1 — Outline to prose

For units marked `outline` in `docs/status.md`:

- Expand planning briefs into continuous prose per `docs/book-rules.md`.
- One unit at a time unless the author approves a batch.
- Reverify 2026 current-event anchors and durability checks before drafting Part I prose.

### Phase 2 — Unit passes (per chapter, bridge, or front/back matter section)

After drafting or revising one unit:

1. **Rules check** — alignment with `docs/book-rules.md`, core invariant, durability/balance gates.
2. **Echo pass** — flag repeated phrasing, claims, and examples against prior units and companion *No Time to Think*.
...
- Structural balance of rules vs. judgment across Part I
...
- Move or copy into `books/the-case-that-does-not-fit/`
