# What We Cannot See — Drafting Status

## Current phase

**Phase 3 — Part III prose (Integration: Learning Together)**

Part I + Part II on `main`. Part III Ch 9–13 drafted on branch; pending author review + part gate.

## Workflow stage (Part III branch)

| Step | Action | Status |
|------|--------|--------|
| — | Ch 9–13 prose | Done (draft) |
| A | Author review final chapter (Ch 13) | Pending |
| D | Manuscript-so-far review (Intro → Ch 13) | Pending |
| E–G | Revise, squash, merge | Pending |

**Branch:** `cursor/what-we-cannot-see-part3-draft-2512` · PR #220

## Part III arc (locked)

| Ch | Question | Status |
|----|----------|--------|
| 9 | Why do others keep seeing things I miss? | approved (~1,520w) |
| 10 | How can incomplete perspectives produce better understanding? | approved (~1,200w) |
| 11 | When failure teaches the system | approved (~1,350w) |
| 12 | How do institutions preserve both wisdom and blind spots? | approved (~1,280w) |
| 13 | What happens when the perspective is not human? | draft ~980w |

**Frame:** Integration ≠ collaboration · redistribute perspective · AI = finite perspective

## Unit progress

| Unit | Phase | Notes |
|------|-------|-------|
| Intro → Limits bridge | approved | on `main` ~13,520w |
| Ch 9–12 | approved | Ch 13 draft pending review |
| Epilogue + back matter | scaffold | after Part III merge |

## Next actions

1. **Author:** Review Ch 13
2. **Agent:** Revise from feedback as chapters are approved
3. **Then:** Epilogue draft after Part III merge

## Rough scale

- On `main`: ~13,520 words
- Part III draft (Ch 9–13): ~5,320 words
- **Total drafted:** ~18,840 words (pending epilogue)

## Key docs

[`integration-framework.md`](integration-framework.md) · [`process-design.md`](process-design.md) · [`status.md`](status.md)
