...
def write_index(book_dir: Path, title: str, entries: list[tuple[str, str, str]]) -> None:
    ...
    write_typst_manifest(book_dir, entries)


def write_typst_manifest(book_dir: Path, entries: list[tuple[str, str, str]]) -> None:
    typst_dir = book_dir / "typst"
    typst_dir.mkdir(parents=True, exist_ok=True)
    lines = [
        "// Auto-generated by tools/html_to_observer_patterns.py",
        "",
    ]
    skip = {"front-matter/title-page.md"}
    for _section, _label, rel in entries:
        if rel in skip:
            continue
        path = f'"../{rel}"'
        ...
        lines.append("")
    write_file(typst_dir / "manifest-parts.typ", "\n".join(lines))
