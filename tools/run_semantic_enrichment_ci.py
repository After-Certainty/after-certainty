...
def _ontology_lint_report(repo: Path, book_id: str) -> Path:
    proc = _run(["python3", "tools/lint_semantic_graph.py", "--repo", "."], cwd=repo)
    stamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    out = repo / ENRICHMENT_ROOT / book_id / "lint-reports" / f"ontology-lint-{stamp}.md"
    body = (
        f"# Ontology lint report ({book_id})\n\n"
        f"Generated by `python3 tools/lint_semantic_graph.py` (exit {proc.returncode}).\n\n"
        f"## stdout\n\n```\n{proc.stdout.strip()}\n```\n\n"
        f"## stderr\n\n```\n{proc.stderr.strip()}\n```\n"
    )
...
def _discovery_report(repo: Path, book_id: str) -> Path:
    ...
    lines.extend(
        [
            ...
            "",
            "- Run `python3 tools/infer_semantic_source_links.py --repo . --dry-run` for co-mention hints.",
            "- Fill enrichment drafts via `mise run semantic:enrich-propose -- <book-dir> <agent-type>` "
            "(or `make propose-semantic-enrichment`) + agent briefs.",
            "",
        ]
    )
    _write_report(out, "\n".join(lines))
...
def pr_body(
    *,
    ...
    checklist = (
        "- [ ] Drafts reviewed; fill `items` before `mise run semantic:enrich-promote` "
        "(or `make promote-semantic-enrichment`)\n"
        "- [ ] `mise run semantic:verify-ontology` (or `make verify-semantic-ontology`) after promote\n"
...
def run(
    ...
    *,
    ...
    if not dry_run and not publish_only:
        lint_proc = _run(["python3", "tools/lint_semantic_graph.py", "--repo", "."], cwd=repo)
