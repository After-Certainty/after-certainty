...
description: >-
  Enriches book-scoped semantic graph YAML from manuscript prose, runs
  verify-semantic-ontology, and opens a PR with canonical changes for review.
  Use when enriching semantic/glossary or patterns, semantic enrichment, issue
  #116, or when the user wants enrichment types (including all) for a book.
---
...
| **Enrichment type** | `recognition-signals`, `trajectories`, `manifestations`, `counterbalances`, `questions`, or **`all`** |

Map `book_id` → `BOOK_DIR` (see [reference.md](reference.md)). Accept synonyms (`signals` → `recognition-signals`, etc.).
...
| `questions` | `questions` |
| `all` | all five fields above |

**Do not** use `semantic/_drafts/enrichment/` for this workflow.

### Read sources

1. Canonical entity (`observation`, `shortDefinition`, `relatedPatterns`, existing enrichment).
...
**After Certainty + `all`:** you may run `python3 tools/apply_after_certainty_book_enrichment.py` instead of hand-editing each file, then still read the manuscript for gaps.
...
git add semantic/glossary semantic/patterns semantic/situations
