#!/usr/bin/env bash
# Install released PADE v0.3.0 for Cloud Agent sessions (idempotent, non-interactive).
set -euo pipefail
...
case "$(uname -s)-$(uname -m)" in
  ...
  *)
    ...
    echo "hint: PADE v0.3.0 release supports linux-amd64, linux-arm64, darwin-arm64" >&2
    exit 1
    ;;
esac
...
BASE="https://github.com/After-Certainty/pade/releases/download/${VERSION}"
