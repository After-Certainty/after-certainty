...
- Concepts, thinkers, sources, situations, questions, and reader trails mobile redesign is an **active specialized site plan** (Phase 0 complete; Phases 1–6 + Phase 1b thinkers/sources type·kind filter+sort planned). See [`apps/site/docs/roadmaps/explore-discovery-mobile-redesign.md`](../../apps/site/docs/roadmaps/explore-discovery-mobile-redesign.md). Type/kind catalog controls are tracked there; **PROVENANCE-004** remains the separate thinkers-by-**concept** filter. Promote unfinished follow-ups here only if they outgrow site-only UX work.
