...
**Status:** Specialized plan — **platform complete (Phases 0–6)**; CBC full-book audio **merged**; WMSS + WOLTY Companion enablement **authored** (pending generate); Phase 8 = ongoing selective narration ops; Phase 7 (OpenAI) = **triggered only**  
**Created:** 2026-08-03  
**Revised:** 2026-09-05 (WMSS + WOLTY Companion full-book audio enablement; OpenAI triggered only per master roadmap prune)  
...
| **Public / production Listen** | **Live** | Available-unit gate (no site env flag). OP + WOLTY v1 + CBC full books generated/merged; WMSS + WOLTY Companion audio-enabled (pending generate). |
| **Pilot / expansion books** | **OP + WOLTY v1 + CBC + WMSS + WOLTY Companion** | Observer Patterns **29/29** available; When Others Look to You v1 **25/25** available; Curiosity Before Certainty **14/14** available; When Moral Seriousness Scales **17/17** audio-enabled (pending generate); WOLTY Companion **18/18** audio-enabled (pending generate). Further books = credit-bounded Ongoing ops. |
...
| AUDIO-P8-03 (CBC full book) | **Done** | Curiosity Before Certainty **all 14** exported units generated and available. |
| AUDIO-P8-04 (WMSS + WOLTY Companion) | **Enablement done; generate pending** | When Moral Seriousness Scales **17/17** + When Others Look to You Companion (v2) **18/18** `audio.enabled: true` (~22.6k Flash credits @ 0.25/char). Pending **Chapter audio generate**. |
...
### Phase 7 — Second-provider evaluation (triggered only)

- **Trigger:** Concrete ElevenLabs cost, quality, reliability, licensing, or alignment/timing problem — not provider plurality for its own sake.
...
### Phase 8 — Expansion (ongoing selective narration ops)

- Enable selected additional units or books when credits and editorial priority justify (not a completeness mandate).
- Monitor API spend, LFS growth, bandwidth.
- Reassess storage/CDN only if a real operational problem appears.
- Platform engineering is complete; this phase is **operations**, not open product backlog.
...
| Production Listen before go-ahead | **Unblocked (2026-08-05)** — Kevin confirmed Starter licensing; OP + WOLTY v1 + CBC full books live; WMSS + WOLTY Companion audio-enabled (pending generate). Further narration is Ongoing ops. |
