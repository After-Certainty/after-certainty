...
- `index.md` links all parts, bridges, and chapters together.
- The manuscript structure must follow the part/chapter file rules in `docs/book-rules.md`.

## Step 2: Draft Sequentially

Draft one unit at a time for review:

- Either one part-level bridge section
- Or one chapter
...
After drafting each part bridge or chapter, run a compliance check against `docs/book-rules.md`.
...
- For chapter drafts, confirm the chapter ends with a pull-quote
...
- Pull-quote formatting consistency (no bold text inside pull-quotes)
...
- Check bridge quality: does the section set up the next section cleanly?
...
After each draft and revision, run a citation pass before the glossary pass.

At minimum:

- Add citation markers at major structural pivots and non-obvious claims
- Use stable, chapter-scoped footnote IDs (for example, `[^c1-coupling-definition]`)
...
- Track chapter-level citation density toward the target range in `docs/book-rules.md` (6-12 meaningful citations per chapter)
- If a source detail is not yet verified, mark it explicitly in the footnote as "verify source details"

## Step 8: Glossary Pass

After each draft and revision, run a glossary pass for cross-domain readability.

At minimum:

- Identify new domain-specific terms introduced in the section
- Add missing terms to `glossary.md` with concise definitions
- Ensure glossary-term bolding follows first occurrence in full manuscript reading order (starting after `Typographical Conventions`), not first occurrence per section
- Ensure terms already introduced earlier in the manuscript are not re-bolded in later sections unless emphasis is explicitly needed
- Ensure glossary definitions do not rely on terms defined later (no forward dependencies)
- If two glossary terms become circularly dependent, pause and ask the author how to untangle the definitions

## Step 9: Linkage Check
...
## Step 11: Human Review and Revision Loop
...
Once all front matter, bridges, chapters, and back matter are drafted and approved, run a final manuscript-wide pass.

### 13.1 Structural Cohesion Pass

- Verify part-to-part progression and bridge continuity
- Confirm each chapter advances the invariant and does not duplicate adjacent chapters
- Check that section headings and sub-headings support navigation across the full manuscript

### 13.2 Global Echo and Compression Pass
...
### 13.4 Citation Integrity and Density Pass
...
- Check chapter-level citation density against the target range in `docs/book-rules.md`

### 13.5 Glossary Completion Pass

- Review all drafted files for domain-specific terms
- Ensure first-use bolding is applied consistently by manuscript reading order
- Expand `glossary.md` so cross-domain readers can resolve unfamiliar terms quickly
- Reorder glossary entries to remove forward dependencies and prevent circular references

### 13.6 Bibliography Completion Pass
...
### 13.7 Final Linkage and Back Matter Integrity Pass

- Confirm all links in `index.md` and across manuscript files resolve correctly
- Validate back matter ordering and completeness (Epilogue, Glossary, Bibliography)
