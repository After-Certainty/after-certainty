#!/usr/bin/env bash
# GA4 Data API getMetadata via process-scoped Material from pade exec.
#
# Requires:
...
if [[ "$GA_ACCESS_TOKEN" == ya29.pade_fake_* ]]; then
  echo '{"kind":"analyticsData#Metadata","dimensions":[],"metrics":[],"note":"fake token; skipped real API"}'
  exit 0
fi
...
http_code="$(
  curl -sS -o "$tmp" -w '%{http_code}' \
    ...
    -H "User-Agent: after-certainty-ga4-get-metadata" \
    "${API_URL}/v1beta/${PROP}/metadata"
)"

if [[ "$http_code" != "200" ]]; then
  echo "error: Google Analytics Data API metadata returned HTTP ${http_code}" >&2
  if [[ -s "$tmp" ]]; then
