...
export function FeatureCard({ title, description, icon, href, className }: FeatureCardProps) {
  const body = (
    <>
      <div
        className="pointer-events-none absolute inset-0 rounded-sm opacity-[0.04] md:opacity-[0.05]"
        aria-hidden
      >
      ...
      <div className="relative flex gap-4">
        <div
          className={cn(
            "flex h-10 w-10 shrink-0 items-center justify-center rounded-sm border border-border/30 bg-bg/[0.35] text-muted",
            href &&
              "transition-colors duration-300 group-hover:border-accent/22 group-hover:text-accent",
          )}
          aria-hidden
        >
          {icon}
  ...
  return (
    <article
      className={cn(
        "relative rounded-sm border border-border/35 bg-bg-elevated/[0.06] p-6 md:p-7",
        className,
      )}
    >
      {body}
