...
const overviewsFromGraph = graph ? bookOverviewsFromGraph(graph) : [];
...
describe.skipIf(!graph)("book overview health (local manifest)", () => {
  it("accepts overviews from the semantic manifest against the graph", () => {
    const issues = collectBookOverviewHealthIssues({ graph: graph!, overviews: overviewsFromGraph });
  ...
  it("covers every priority slug exactly once", () => {
    ...
    const parsed = getBookOverviewsManifest(graph!);
