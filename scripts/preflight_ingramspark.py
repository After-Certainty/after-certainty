#!/usr/bin/env python3
"""
Unified IngramSpark preflight (profile-driven JSON + human report).

Does not assemble a ZIP (see package_ingramspark.py). Ebook-only mode works
without print assets.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Reuse existing build/ingramspark outputs; do not export ebook/print first",
    )
    parser.add_argument(
        "--allow-cover-upscale",
        action="store_true",
        help="Test/fixture only; do not use for production",
    )
    ...
    if not args.skip_build:
        ...
        ebook_on = isinstance(target.get("ebook"), dict) and target["ebook"].get("enabled") is True
        print_on = isinstance(target.get("print"), dict) and target["print"].get("enabled") is True
