...
def test_stage_pdf_units_keeps_distinct_bridge_files(tmp_path: Path) -> None:
    ...
    # Bridges stay distinct paths and keep their body; PDF staging top-aligns them.
    assert "# Part I bridge" not in text1  # heading promoted to LaTeX
    assert r"\Large\bfseries Part I bridge" in text1
    assert r"\Large\bfseries Part V bridge" in text5
    assert "\\vspace*{2.45in}" in text1
    assert "\\vspace*{2.45in}" in text5
